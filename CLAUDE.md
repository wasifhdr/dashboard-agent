...
| `actionSchema.js` | zod discriminated union — 9 action types (`set_filter`, `set_range_filter`, `set_parameter`, `switch_sheet`, `wait`, `answer`, `fail`, `click`, `scroll`). `click` and `scroll` are **pixel-mode only**; `scroll` is vertical-only and carries no magnitude (the actuator supplies `pixel.scrollNotchPx`). **None advances a story point.** |
