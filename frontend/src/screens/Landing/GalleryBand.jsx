...
function DashboardThumbnail({ dashboard }) {
  if (dashboard.thumbnailUrl) {
    return (
      <div className="glass aspect-[16/10] overflow-hidden rounded-control">
        <img src={dashboard.thumbnailUrl} alt="" className="h-full w-full object-cover" />
      </div>
    );
  }
  return (
    <div className="glass flex aspect-[16/10] items-center justify-center rounded-control">
      <span className="font-sans text-display-sm font-extrabold text-mist/25">{dashboard.name.charAt(0)}</span>
    </div>
  );
}
...
function DashboardCard({ dashboard, onOpen }) {
  return (
    <Card variant="clickable" onClick={() => onOpen(dashboard.url, dashboard.name)}>
      ...
      <p className="mt-1 text-sm text-mist/70">{dashboard.description}</p>
  ...
  return (
    <section>
      <div className="mx-auto max-w-6xl px-6 py-16">
        <CapsLabel className="text-teal">GALLERY</CapsLabel>
        <h2 className="mt-2 text-h1">{heading}</h2>
        {!isSearching && (
          <p className="mt-2 text-sm text-mist/70">Curated from the DashboardQA benchmark and validated end-to-end.</p>
        )}
        ...
        <div className="mt-8">
          {isUrlQuery ? (
            ...
            <EmptyState
              statement={
                <>
                  No dashboard knows about that <span className="text-gold">yet.</span>
                </>
              }
